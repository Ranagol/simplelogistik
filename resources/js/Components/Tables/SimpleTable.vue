...
import SearchFilter from '../Inputs/SearchFilter.vue';
    ...
    const headerHelper = new TableHeaders(props.tableConfig ?? [], props.configName ?? (route().current() + "-headers"));
...
<template>
    <div class="relative mt-5 bg-white dark:bg-gray-800 sm:rounded-lg">
        <div class="grid justify-between grid-flow-col py-3">
            <div>
                <SearchFilter v-if="showSearch" :searchConfig="searchConfig" />
            </div>
            <div>
                <button v-if="showFilter" id="actionsDropdownButton" data-dropdown-toggle="actionsDropdown" class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg md:w-auto focus:outline-none hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" type="button">
                    <svg class="-ml-1 mr-1.5 w-5 h-5" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <path clip-rule="evenodd" fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
                    </svg>
                    {{ $t('buttons.general.arrange') }}
                </button>
                <div v-if="showFilter" id="actionsDropdown" class="z-50 hidden bg-white divide-y divide-gray-100 rounded shadow dark:bg-gray-700 dark:divide-gray-600">
