    ...
    <section class="bg-gray-50 dark:bg-gray-900">
            ...
            <div class="relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                <div
                    ...
                    <FilteredSearch searchAt="forwarders.index" :headers="_headers" />
                    
                    <div class="grid grid-flow-col gap-4">
                        <!-- CREATE ORDER BUTTON -->
                        <button @click="router.visit(route('forwarders.create'))" type="button"
                            ...
                            {{ $t('page-actions.forwarder-create') }}
                        </button>
                        ...
                        <div id="showTableColumns"
                            ...
                            <h6 class="mb-3 overflow-y-scroll text-sm font-medium text-gray-900 max-h-48 dark:text-white">{{ $t('labels.select-fields')}}</h6>
                                    ...
                                    <div :id="'actions-dropdown-' + entry.id"
                                        ...
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                            ...
                                            <li v-for=" action  in  actions ">
                                                <a @click.prevent="router.visit(route(`forwarders.show`, entry.id ))" v-if="action === 'show'" href="#"
                                                    class="block w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{
                                                        $t('labels.show') }}</a>
                                                <a @click.prevent="router.visit(route(`forwarders.edit`, entry.id ))" v-else-if="action === 'edit'" href="#"
                                                    class="block w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{
                                                        $t('labels.edit') }}</a>
                                                <button @click="router.visit(route(`forwarders.destroy`, entry.id ))" v-if="action === 'delete'" href="#"
...
var tableHeaders = new TableHeaders(props.headers, "forwarder-table-headers");
